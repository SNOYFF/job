var tagsalary = new Vue({
	...
	methods:{
		tagsalarys:function(){
			var that = this;
			axios.get('http://114.55.104.213:8080//queryJob_tagSalary').then(function(response){
				/* console.log(response); */
				for(var i = 0;i<response.data.length;i++){
					that.jobtag.push(response.data[i].jobtag);
					that.avgsalary.push(response.data[i].avgsalary);
				}
				 var myChart = echarts.init(document.getElementById('tagsalary'));
				var option = {
				title: {
				    //text: '岗位种类-薪酬', //标题文本内容
					subtext: '单位：K/月'
